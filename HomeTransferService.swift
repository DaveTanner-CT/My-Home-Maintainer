...
enum HomeTransferService {
    ...
    static func validateForImport(_ archive: HomeTransferArchive) throws {
        ...
        func valid(_ ids: [String]?, in set: Set<String>) -> Bool { (ids ?? []).allSatisfy { set.contains($0) } }
    ...
    static func importIntoEmptyStore(_ archive: HomeTransferArchive, context: ModelContext) throws {
        ...
        for s in archive.systems {
            ...
            item.additionalRooms = (s.additionalRoomIDs ?? []).compactMap { rooms[$0] }
        ...
        for a in archive.appliances {
            ...
            item.additionalRooms = (a.additionalRoomIDs ?? []).compactMap { rooms[$0] }
        ...
        for t in archive.tasks {
            ...
            item.additionalRooms = (t.additionalRoomIDs ?? []).compactMap { rooms[$0] }
    ...
    private static func makeArchive(context: ModelContext, packageType: String, includedApplianceIDs: Set<PersistentIdentifier>?, includedFixtureIDs: Set<PersistentIdentifier>?, includedFurnitureIDs: Set<PersistentIdentifier>?) throws -> HomeTransferArchive {
        ...
        return HomeTransferArchive(
            formatVersion: 1, appVersion: "0.37.1", packageType: packageType, exportedAt: .now, home: h,
