...
struct DetectorSnapshot: Codable { let location, roomName, type, manufacturer, model, batteryType, notes: String; let manufactureDate, installationDate, replacementDate: Date?; let isHardwired: Bool }
struct ConsumableSnapshot: Codable { let name, roomName, type, size, manufacturer, modelPartNumber, purchaseLink, notes: String; let replacementIntervalMonths: Int?; let lastReplaced, nextReplacement: Date? }
...
enum HomeExportService {
    static func makeArchive(context: ModelContext) throws -> HomeArchive {
        ...
        return HomeArchive(
            ...
            consumables: consumables.map { .init(name: $0.name, roomName: $0.room?.name ?? "", type: $0.type, size: $0.size, manufacturer: $0.manufacturer, modelPartNumber: $0.modelPartNumber, purchaseLink: $0.purchaseLink, notes: $0.notes, replacementIntervalMonths: $0.replacementIntervalMonths, lastReplaced: $0.lastReplaced, nextReplacement: $0.nextReplacement) },
            projects: projects.map { .init(title: $0.title, projectDescription: $0.projectDescription, stage: $0.stage.rawValue, notes: $0.notes, roomName: $0.linkedRooms.isEmpty ? $0.locationName : $0.linkedRooms.map(\.name).joined(separator: ", "), targetDate: $0.targetDate, budget: $0.budget) },
