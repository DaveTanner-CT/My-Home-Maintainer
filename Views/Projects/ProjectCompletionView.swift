...
struct ProjectCompletionView: View {
    ...
    private var purchased: [ProjectItem] {
        allItems.filter { $0.project?.persistentModelID == project.persistentModelID && $0.status == .purchased && !$0.isIdeaOnly }
    }
    ...
    var body: some View {
        List {
            ...
            Section("Purchased Items") {
                if purchased.isEmpty { Text("No purchased project items to transfer.").foregroundStyle(.secondary) }
                ForEach(purchased) { item in
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Menu {
                            Button { addAppliance(item) } label: { Label("Appliance / Equipment", systemImage: "refrigerator") }
    ...
    private func addAppliance(_ item: ProjectItem) {
        ...
        modelContext.insert(record); copyPhoto(item, to: .appliance(record)); save()
    }
    private func addFixture(_ item: ProjectItem) {
        ...
        modelContext.insert(record); copyPhoto(item, to: .fixture(record)); save()
    }
    private func addSystem(_ item: ProjectItem) {
        ...
        modelContext.insert(record); copyPhoto(item, to: .system(record)); save()
    }
    private func addPaint(_ item: ProjectItem) {
        ...
        modelContext.insert(record); copyPhoto(item, to: .paint(record)); save()
    }
    private func addHistory(_ item: ProjectItem) {
        ...
        modelContext.insert(record); copyPhoto(item, to: .maintenanceRecord(record)); save()
    }
