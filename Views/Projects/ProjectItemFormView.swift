...
struct ProjectItemFormView: View {
    ...
    @State private var comparisonGroup: String
    @State private var isIdeaOnly: Bool
    ...
    private let categories = [
        "Inspiration", "Paint & Colors", "Flooring / Tile", "Fixtures", "Lighting", "Hardware",
        "Furniture", "Storage", "Appliances", "Electronics", "Home Technology", "Tools / Equipment",
        "Outdoor Equipment", "Materials", "Contractor Ideas"
    ]

    init(project: Project, existing: ProjectItem? = nil, initialComparisonGroup: String = "", initialCategory: String = "Inspiration") {
        self.project = project
        self.existing = existing
        _title = State(initialValue: existing?.title ?? "")
        _category = State(initialValue: existing?.category ?? initialCategory)
        _comparisonGroup = State(initialValue: existing?.comparisonGroup ?? initialComparisonGroup)
        _isIdeaOnly = State(initialValue: existing?.isIdeaOnly ?? false)
        _manufacturer = State(initialValue: existing?.manufacturer ?? "")
        _model = State(initialValue: existing?.model ?? "")
        _sku = State(initialValue: existing?.sku ?? "")
        ...
        _unitCostText = State(initialValue: existing?.unitCost.map { String($0) } ?? "")
        _quantity = State(initialValue: existing?.quantity ?? 1)
        _actualCostText = State(initialValue: existing?.actualPurchaseCost.map { String($0) } ?? "")
        _purchaseDate = State(initialValue: existing?.purchaseDate ?? .now)
        _notes = State(initialValue: existing?.notes ?? "")
        _status = State(initialValue: existing?.status ?? .considering)
    ...
    var body: some View {
        Form {
            Section("Idea / Item") {
                TextField("Option name", text: $title)
                Picker("Category", selection: $category) {
                    ForEach(categories, id: \.self) { Text($0).tag($0) }
                }
                Toggle("Idea only", isOn: $isIdeaOnly)
                Picker("Status", selection: $status) {
                    ForEach(ProjectItemStatus.allCases) { Text($0.rawValue).tag($0) }
                }
            }

            if !isIdeaOnly {
                Section("Buying Decision") {
                    TextField("What are you choosing?", text: $comparisonGroup, prompt: Text("e.g. Kitchen Faucet"))
                    Text("Use the same decision name for competing options. Shopping and Compare Options will keep those choices together.")
                        .font(.footnote)
                ...
                Section("Product") {
                    ...
                    if status == .purchased || status == .installed {
                        DatePicker("Purchase date", selection: $purchaseDate, displayedComponents: .date)
                        TextField("Actual purchase cost", text: $actualCostText).keyboardType(.decimalPad)
                    }
                }
            }
            ...
            if existing != nil {
                Section { Button("Delete Project Item", role: .destructive) { showDelete = true } }
            }
        }
        .navigationTitle(existing == nil ? "Add Option / Item" : "Edit Option / Item")
    ...
    private func save() {
        let item = existing ?? ProjectItem(project: project, title: title)
        if existing == nil { modelContext.insert(item) }
        item.project = project
        item.title = title
        item.category = category
        item.comparisonGroup = comparisonGroup.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? nil : comparisonGroup.trimmingCharacters(in: .whitespacesAndNewlines)
        item.isIdeaOnly = isIdeaOnly
        item.manufacturer = manufacturer
        item.model = model
        item.sku = sku
        item.finishColor = finishColor
        item.dimensions = dimensions
        item.store = store
        item.website = website
        item.unitCost = Double(unitCostText)
        item.quantity = quantity
        item.actualPurchaseCost = Double(actualCostText)
        item.purchaseDate = (status == .purchased || status == .installed) ? purchaseDate : nil
        item.notes = notes
        item.status = status
